...
func (t *ABC) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	err = stub.CreateTable("ITEM", []*shim.ColumnDefinition{
		...
		&shim.ColumnDefinition{Name: "invNumber", Type: shim.ColumnDefinition_STRING, Key: false},
	...
	// setting up the users role
    stub.PutState("STATUS_SHT_WAR", []byte("Line shipped to Warehouse"))
...
//get getLineitemByExpDate(irrespective of organization)
func (t *ABC) getLineitemByExpDate(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for row := range rows {		
		fetchedLineItemExpDate := row.Columns[19].GetString_()
		
		if fetchedLineItemExpDate == ExpDate{
...
func (t *ABC) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
    ...
    }else if function == "getLineitemByExpDate" { 
		t := ABC{}
		return t.getLineitemByExpDate(stub, args)
	}else if function == "getLineitemByinvno" { 
		t := ABC{}
		return t.getLineitemByinvno(stub, args)
